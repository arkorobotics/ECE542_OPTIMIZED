...
/////////////////////////////////////////////////////////////
// 
// CAL POLY POMONA - ECE542 FINAL PROJECT
//
// Author: Ara Kourchians
//
// Vision Processing Code: Ara Kourchians
//
// Camera Serial Interface Example Code: Pierre Raufast 
...
Mat framebuffer,frame,face,face_resized;
vector<Mat> images;
vector<int> labels;
...
typedef struct
{
   ...
   VCOS_SEMAPHORE_T complete_semaphore; // semaphore which is posted when we reach end of frame 
   ...
   if (pData)
   {
      ...
      if (buffer->length)
      {
		...
		if (pData->pstate->graymode==0)
		{
			...
			cvCvtColor(image,dstImage,CV_YCrCb2BGR);	// convert in RGB color space (slow)
			cvCvtColor(dstImage,dstImage,CV_RGB2BGR);			
			framebuffer = cvarrToMat(dstImage);   
			
		}
		else
		{	
			// gray mode
			framebuffer = cvarrToMat(py);  
		}
		
		////////////////////////////////
		// ECE542 CODE START HERE
		////////////////////////////////

		// Input Image Frame BGR: dstImage
		
		// Create HSV Image
		IplImage* imgHSV = cvCreateImage(cvGetSize(dstImage), 8, 3);

		// Create Threshold Image		
		IplImage* imgThreshold = cvCreateImage(cvGetSize(dstImage), 8, 1);

		// Convert BGR Image to HSV
		cvCvtColor(dstImage,imgHSV,CV_BGR2HSV);

		// Apply Threshold looking for green hue, with large saturation & intensity limits
		cvInRangeS(imgHSV, cvScalar(45, 130, 100), cvScalar(85, 250, 250), imgThreshold);		
		
		// Run through the entire image looking for green pixels (any pixel value greater
		// greater than 0). Check the pixel x,y to see if it's larger or smaller than
		// the current max or min values for x or y. If so, set the new max or min.
		int pixel = 0; 
		for(int h = 0; h <  cvGetSize(imgThreshold).height; h++)
		{
			for(int w = 0; w < cvGetSize(imgThreshold).width; w++)
			{
				pixel = CV_IMAGE_ELEM(imgThreshold, unsigned char, h, w);
				if(pixel > 0)
				{
					if(w < x_min) { x_min = w; }
					if(w > x_max) { x_max = w; }
		...
		// Draw Rectangle around min,max dimensions of the blob
		cvRectangle(dstImage, rect_tl, rect_br, cvScalar(255,255,255,0), 1, 8, 0);
		
		// Draw Cross Hairs at the center of the green blob
		cvLine(	dstImage, 
			cvPoint((int)( x_min+(x_max-x_min)/2)-5 , (int)( y_min+(y_max-y_min)/2) ), 
			cvPoint((int)( x_min+(x_max-x_min)/2)+5 , (int)( y_min+(y_max-y_min)/2) ), 
		     	cvScalar(255,255,255), 
			1);	
	
		cvLine(	dstImage, 
			cvPoint((int)( x_min+(x_max-x_min)/2) , (int)( y_min+(y_max-y_min)/2)-5 ), 
			cvPoint((int)( x_min+(x_max-x_min)/2) , (int)( y_min+(y_max-y_min)/2)+5 ), 
		     	cvScalar(255,255,255),
			1);	
		
		// Calculate x,y center of green object	
		x_center = (int)( x_min+(x_max-x_min)/2);
		y_center = (int)( y_min+(y_max-y_min)/2);

		// Display the color image with the rectangle and cross hairs
		framebuffer = cvarrToMat(dstImage); 
		
		// Print out minimums, maximums, and center coordinates
		vcos_log_error("x_min: %d, x_max: %d, y_min: %d, y_max: %d, x_ctr: %d, y_ctr: %d\n", 
	...
	// Show the result:
	imshow("ECE542 Camera", framebuffer);
	key = (char) waitKey(1);
...
int main(int argc, const char **argv)
{
	...
	cvNamedWindow("ECE542 Camera", CV_WINDOW_AUTOSIZE); 
	int w=state.width;
	int h=state.height;
